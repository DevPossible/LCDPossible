...
/// <summary>
/// Proxmox VE API provider for cluster monitoring.
/// </summary>
public sealed class ProxmoxProvider : IProxmoxProvider
{
    private readonly ProxmoxOptions _options;
    private readonly ILogger<ProxmoxProvider>? _logger;
    ...
    public string Name => "Proxmox VE";
    public bool IsAvailable => _initialized && !_disposed && _httpClient != null;

    public ProxmoxProvider(IOptions<LcdPossibleOptions> options, ILogger<ProxmoxProvider>? logger = null)
    {
        _options = options?.Value?.Proxmox ?? throw new ArgumentNullException(nameof(options));
        _logger = logger;
    }
    ...
    public Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _initialized = true;
            _logger?.LogInformation("Proxmox provider initialized for {ApiUrl}", _options.ApiUrl);
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "Failed to initialize Proxmox provider");
            throw;
        }
    ...
    {
        // Convert Proxmox metrics to generic system metrics
        var proxmox = await GetProxmoxMetricsAsync(cancellationToken);
        if (proxmox == null)
        {
            return null;
        }
        ...
        {
            Timestamp = proxmox.Timestamp,
            Cpu = new CpuMetrics
            {
                Name = $"Proxmox Cluster ({proxmox.ClusterName})",
                UsagePercent = proxmox.Summary.CpuUsagePercent
            },
            Memory = new MemoryMetrics
            {
                UsagePercent = proxmox.Summary.MemoryUsagePercent
            }
        };
    }

    public async Task<ProxmoxMetrics?> GetProxmoxMetricsAsync(CancellationToken cancellationToken = default)
    {
        if (!IsAvailable)
        {
            return null;
    ...
    public async Task<ProxmoxVmMetrics?> GetVmStatusAsync(string node, int vmId, CancellationToken cancellationToken = default)
    {
        if (!IsAvailable)
        {
            return null;
        ...
        try
        {
            var response = await GetAsync<ProxmoxApiResponse<VmStatusItem>>(
                $"nodes/{node}/qemu/{vmId}/status/current", cancellationToken);

            if (response?.Data != null)
            {
                var vm = response.Data;
                return new ProxmoxVmMetrics
                {
                    VmId = vmId,
        ...
        {
            _logger?.LogWarning(ex, "Failed to get VM {VmId} status on node {Node}", vmId, node);
        }

        return null;
    }

    public async Task<ProxmoxContainerMetrics?> GetContainerStatusAsync(string node, int containerId, CancellationToken cancellationToken = default)
    {
        if (!IsAvailable)
        {
            return null;
        ...
        try
        {
            var response = await GetAsync<ProxmoxApiResponse<ContainerStatusItem>>(
                $"nodes/{node}/lxc/{containerId}/status/current", cancellationToken);

            if (response?.Data != null)
            {
                var ct = response.Data;
                return new ProxmoxContainerMetrics
                {
        ...
        {
            _logger?.LogWarning(ex, "Failed to get container {ContainerId} status on node {Node}", containerId, node);
        }
    ...
    public void Dispose()
    {
        ...
        _httpClient?.Dispose();
        _httpClient = null;
        _logger?.LogInformation("Proxmox provider disposed");
    }
    ...
    private sealed class VmStatusItem
    {
        public string? Name { get; set; }
        public string Status { get; set; } = string.Empty;
        public double Cpu { get; set; }
    ...
    private sealed class ContainerStatusItem
    {
        public string? Name { get; set; }
        public string Status { get; set; } = string.Empty;
        public double Cpu { get; set; }
