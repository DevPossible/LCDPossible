#!/bin/bash
# LCDPossible Uninstaller for Ubuntu/Debian/Proxmox
# Usage: curl -fsSL https://raw.githubusercontent.com/USER/LCDPossible/main/scripts/uninstall-ubuntu.sh | sudo bash
#
# This script:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --quiet|-q)
            ...
            ;;
        --help|-h)
            echo "LCDPossible Uninstaller for Ubuntu/Debian/Proxmox"
            echo ""
...
echo ""
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║${NC}           LCDPossible Uninstaller - Ubuntu/Debian            ${BLUE}║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
info "Dependencies (vlc, fonts-dejavu-core, jq) were not removed."
info "Remove them manually if no longer needed:"
echo "    sudo apt remove vlc fonts-dejavu-core jq"
echo ""
